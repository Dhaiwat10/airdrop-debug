...
function Index() {
  ...
  useAsync(async () => {
    if (wallet) {
      ...
      const testContractInstanceRead = new TestContract(contractId, wallet.provider);

      // Read the current value of the counter
      const { value } = await testContractInstanceRead.functions.get_count().get();
  ...
  return (
    <>
      ...
      <>
        ...
        <Button onClick={getOwner} className="mt-6">
          Get Owner
        </Button>
      </>
